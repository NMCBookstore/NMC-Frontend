...
import { close,signup } from '../../slice/ShowLoginSlide';
const BdxLogModal: React.FunctionComponent = () => {
    const showLog = useSelector((state: RootState) => state.showLogin.status);
    const cancelButtonRef = useRef(null)
    const dispatch = useDispatch();
    return (
        <Transition.Root show={showLog == "none" ? false : true } as={Fragment}>
        <Dialog as="div" className="relative z-10" initialFocus={cancelButtonRef} onClose={() => dispatch(close())}>
            <Transition.Child
                as={Fragment}
                ...
                <div className="flex min-h-full items-end justify-center p-4 text-center items-center sm:p-0">
                    ...
                    >
                        <Dialog.Panel className="relative transform overflow-hidden rounded-[48px] bg-[#fff] text-left shadow-xl transition-all sm:my-8 w-full max-w-[440px] px-5 py-10 sm:px-10">
                            <div className="mb-4">
                                <div className="sm:flex sm:items-start">
                                    <div className="text-center w-full">
                                        <Dialog.Title as="h3" className="mb-4 font-semibold leading-normal capitalize text-[40px] text-[#0F3BB0]">
                                            {showLog == 'login' && "Login"}
                            ...
                            <div className="mb-4">
                                <button type="submit" className="w-full py-3 px-6 block uppercase bg-[#F7C937] rounded-full mb-4">
                                    {showLog == 'login' && "Login"}
                                    {showLog == 'signup' && "Sign Up"}
                            ...
                            <button className="btn-close absolute top-10 right-10 z-11 focus-visible:outline-0"
